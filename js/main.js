...
const I18N = {
  pt: {
    ...
    "case.next.dm": "Próximo case: Datamart em Star Schema →",
    ...
    "case.dm.title": "Datamart Analítico em Star Schema",
    "case.dm.subtitle": "Modelagem dimensional completa — de decisões de granularidade a pipelines de carga e documentação — para servir análises do mercado financeiro.",
    "case.dm.tag4": "Modelagem Dimensional",
    "case.dm.tag5": "Documentação",
    "case.dm.link1": "Diagrama interativo (dbdocs)",
    "case.dm.link2": "Documentação (Notion)",
    "case.dm.ctx": "Quando as áreas de negócio consomem dados direto dos sistemas transacionais, cada análise reinventa as regras: queries pesadas, métricas que não batem entre relatórios e nenhuma visão histórica confiável. A resposta clássica de engenharia analítica é separar o consumo em um datamart dimensional — e este projeto percorre esse caminho de ponta a ponta.",
    "case.dm.sol": "Um datamart em Star Schema: tabela fato com granularidade explícita cercada por dimensões com chaves substitutas (surrogate keys), carregado por pipelines ETL com nomenclatura padronizada. Cada decisão de modelagem — o que é fato, o que é dimensão, qual o grão de uma linha — foi documentada em modelo lógico, dicionário de dados e fluxo de cargas.",
    "case.dm.fig1": "Visão conceitual do modelo — o diagrama completo, com todas as colunas e relacionamentos, está publicado no dbdocs.",
    "case.dm.h1": "Granularidade definida antes de qualquer tabela: cada linha da fato representa um evento no seu menor nível útil de análise — a decisão que governa todo o modelo.",
    "case.dm.h2": "Chaves substitutas em todas as dimensões, desacoplando o modelo analítico das chaves naturais dos sistemas de origem.",
    "case.dm.h3": "Pipelines ETL de carga e atualização com nomenclatura padronizada — legibilidade e manutenção como requisito, não acidente.",
    "case.dm.h4": "Documentação tratada como entregável: modelo lógico, dicionário de dados e fluxo de cargas publicados (dbdocs + Notion).",
    "case.dm.h5": "Modelo pronto para consumo por dashboards e análises recorrentes sem retrabalho de regra de negócio.",
    ...
    "case.ata.fig2": "Formato da ata gerada — exemplo ilustrativo com dados fictícios.",
  ...
  en: {
    ...
    "case.next.dm": "Next case: Star Schema Data Mart →",
    ...
    "case.dm.title": "Analytical Data Mart in Star Schema",
    "case.dm.subtitle": "Complete dimensional modeling — from granularity decisions to load pipelines and documentation — serving financial market analytics.",
    "case.dm.tag4": "Dimensional Modeling",
    "case.dm.tag5": "Documentation",
    "case.dm.link1": "Interactive diagram (dbdocs)",
    "case.dm.link2": "Documentation (Notion)",
    "case.dm.ctx": "When business teams consume data straight from transactional systems, every analysis reinvents the rules: heavy queries, metrics that disagree across reports and no reliable historical view. The classic analytics-engineering answer is to separate consumption into a dimensional data mart — and this project walks that path end to end.",
    "case.dm.sol": "A Star Schema data mart: a fact table with explicit granularity surrounded by dimensions with surrogate keys, loaded by ETL pipelines with standardized naming. Every modeling decision — what is a fact, what is a dimension, what one row means — was documented in a logical model, data dictionary and load flow.",
    "case.dm.fig1": "Conceptual view of the model — the full diagram, with all columns and relationships, is published on dbdocs.",
    "case.dm.h1": "Granularity defined before any table: each fact row represents an event at its smallest useful level of analysis — the decision that governs the whole model.",
    "case.dm.h2": "Surrogate keys in every dimension, decoupling the analytical model from source-system natural keys.",
    "case.dm.h3": "ETL load and refresh pipelines with standardized naming — readability and maintainability as a requirement, not an accident.",
    "case.dm.h4": "Documentation treated as a deliverable: logical model, data dictionary and load flow published (dbdocs + Notion).",
    "case.dm.h5": "Model ready for consumption by dashboards and recurring analyses with no business-rule rework.",
    ...
    "case.ata.fig2": "Format of the generated minutes — illustrative example with fictitious data.",
